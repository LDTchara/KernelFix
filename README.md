...
- 可在 `BepInEx/config/com.LDTchara.KernelFix.cfg` 中通过 `EnableRamFix` 开关关闭。
...
- 使用 **数字键 1‑9** 或 **空格键** 选词上屏，使用 `[` `]` 来切换候选页数。
...
> *注：原版 HacknetFontReplace 可能无法正确渲染 UTF‑16 代理对字符（如 emoji），导致候选框中显示为空白或引发游戏异常。推荐使用修复版本 [HacknetFontReplace_UTF16Fix](https://github.com/LDTchara/HacknetFontReplace_UTF16Fix)，或避免在终端中打出此类字符。*
...
4. 采用 **Costura.Fody** 将 `ImeSharp` 及其依赖嵌入 DLL，生成的 `KernelFix.dll` 即为独立文件，无需其他附加库。
...
- Toggle via `EnableRamFix` in `BepInEx/config/com.LDTchara.KernelFix.cfg`.
...
- **It is recommended to use KernelFix together with [HacknetFontReplace](https://github.com/LDTchara/HacknetFontReplace_UTF16Fix) (modified version)** to fix missing glyphs (displayed as `?`).
